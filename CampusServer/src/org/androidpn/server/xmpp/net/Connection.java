...
public class Connection {
    ...
    public void deliver(Packet packet) {
       ...
       JID from=packet.getFrom();
        // @author: xzg
        // to prevent server from sending packet to itself
       	// add a recipient check to this function
        if(dest!=null&&!dest.toString().contains("@")&&from!=null&&!from.toString().contains("@")) {//no username
        	log.debug("!!!THIS PACKET IS NOT VALID");
        	this.close();
        	return;
        }
        if (!isClosed()) {
