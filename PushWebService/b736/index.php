...
function findAddress($longitude,$latitude) {   //根据输入经纬度找地址
	...
	if ( $longitude == NULL || $latitude == NULL ) {
		return 0;
	} else if ( abs($longitude-113.974-$delta) <= 0.0001 ) {   
		return 1;
	} else if ( abs($longitude-113.979576-$delta) <= 0.0001 ) {
		return 2;
...
function showTime( $a ) {//输出时间，在25分钟内的输出时间，25分钟外的输出无人分享
	$a = strtotime($a);
	$b = time();
	if ( $b - $a <= 60*25 ) //小于25分钟
	 return date("H:i:s",$a);
...
function showNextTime( $a ) {//输出时间，在25分钟内的输出时间，25分钟外的输出未知
	$a = strtotime($a);
	$b = time();
	if ( $b - $a <= 60*25 ) //小于25分钟
	 return date("H:i:s",$a+25*60);
	else return "未知";
}
...
<?php 
	...
	//测试用的，a208
	$sql3="select * from b736 where destination = 3 order by chtime desc limit 1";
	$result3=mysql_query($sql3,$link_ID);
?>
<showtable1>
  <div>北大园区:</div>
  <div>上一班:<?php if ( $result = mysql_fetch_array($result1)) echo showTime($result['chtime']); else echo "无法确定" ?></div>
  <div>下一班:<?php if ( $result ) echo showNextTime($result['chtime']); else echo "无法确定" ?></div>
  <div>----------------------------</div>
  <div>春园路口:</div>
  <div>上一班:<?php if ( $result = mysql_fetch_array($result2)) echo showTime($result['chtime']); else echo "无法确定" ?></div>
  <div>下一班:<?php if ( $result ) echo showNextTime($result['chtime']); else echo "无法确定" ?></div>
</showtable1>
<showlist1>
...
function showMessage( )//异步输出信息
{
	...
	if ( msgButton == true ) {
		...
		document.getElementById('suggestion').innerHTML = "在公交车开GPS，按实际情况按\"上车\",\"等车\"可分享公交信息<br>您提供的信息将给其他人将给其他人带来极大方便";
